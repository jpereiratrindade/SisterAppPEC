...
// ... generateCubeMesh (Skipping for brevity, assuming standard brace init style which will Zero UVs implicitly if using designated init or if we fix call sites? No, C++ aggregate init is strict if braces are used).

// Actually, generateCubeMesh uses helper function addQuad which I updated above. So it is fine.
// generatePyramidMesh uses helpers too. Fine.

// createSkyDome needs update.
