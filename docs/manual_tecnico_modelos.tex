...
\title{\textbf{SisterApp Engine v3.7.3}\\Manual Técnico de Modelos Computacionais}
...
\section{Introdução}
O \textbf{SisterApp Engine} integra sistemas avançados de análise topológica e persistência de dados. A versão 3.7.3 introduz um novo modelo focado na \textbf{Análise Semântica de Solos (Semantic Soil Analysis)}, garantindo consistência total entre visualização e dados físicos.
...
\subsection{Minimap e Navegação}
A versão 3.8.0 introduz um Minimapa interativo para facilitar a navegação em grandes terrenos.
\begin{itemize}
    \item \textbf{Visualização}: Renderiza o mapa de solos e relevo (hillshade) em tempo real.
    \item \textbf{Navegação}: Permite Zoom (Roda do Mouse) e Pan (Arrastar com botão do meio).
    \item \textbf{Teletransporte}: Clique com botão esquerdo para mover a câmera instantaneamente.
    \item \textbf{Métricas}: Integração direta com o relatório de métricas de paisagem para validação visual.
\end{itemize}
