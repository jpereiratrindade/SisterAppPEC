...
O SisterApp v3.9.1 introduz um modelo de dinâmica de vegetação campestre baseado em princípios de ecologia espacial e regimes de distúrbio (Fogo e Pastejo).
...
\begin{itemize}
    \item \textbf{Estrato Inferior (EI):} Gramíneas e herbáceas. Alta taxa de crescimento, alta resiliência ao pastejo, inflamável.
    \item \textbf{Estrato Superior (ES):} Arbustos e subarbustos. Crescimento lento, suprime o estrato inferior por sombreamento, sensível ao fogo.
\end{itemize}
...
A dinâmica temporal da vegetação é regida por equações diferenciais discretizadas no tempo ($\Delta t \approx 0.1s$).

\subsubsection{Crescimento e Recuperação}
Após o período de latência ($T_{recovery}$), a biomassa do Estrato Inferior ($C_{EI}$) recupera-se linearmente até a capacidade de suporte local ($K_{EI}$):
\[
\frac{dC_{EI}}{dt} = r_{growth} \cdot Vigor_{EI} \quad \text{se} \quad C_{EI} < K_{EI}
\]
Onde $r_{growth} = 0.1 s^{-1}$ representa a taxa de crescimento intrínseca. O Estrato Superior ($C_{ES}$) coloniza a área apenas se houver estabilidade ($C_{EI} > 0.8$) e espaço disponível, simulando a sucessão ecológica secundária.
